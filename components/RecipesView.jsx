...
// Full-screen popup showing all recipe details
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="relative aspect-video bg-slate-900 rounded-t-2xl overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="absolute top-3 right-3 p-2 bg-black/60 hover:bg-black/80 rounded-full transition-colors text-white"
            aria-label="Close"
          >
            <Close size={20} />
          </button>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                <ChefHat size={22} className="text-purple-400 shrink-0" />
                <span>{recipe.name}</span>
              </h2>
              {recipe.prepTime && (
                <p className="text-slate-400 mt-1">⏱ {recipe.prepTime}</p>
              )}
            </div>
            {onEdit && (
              <button
                onClick={() => { onEdit(recipe); onClose(); }}
                className="shrink-0 flex items-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm text-slate-200 transition-colors"
              >
                <span>✎</span> Edit
              </button>
            )}
          ...
          {recipe.ingredients && (
            <div>
              <h3 className="text-sm font-semibold text-purple-300 uppercase tracking-wide mb-2">Ingredients</h3>
              <p className="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed bg-slate-900/50 rounded-xl p-4">
                {recipe.ingredients}
              </p>
            </div>
          )}
          ...
          {recipe.instructions && (
            <div>
              <h3 className="text-sm font-semibold text-purple-300 uppercase tracking-wide mb-2">Instructions</h3>
              <p className="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed bg-slate-900/50 rounded-xl p-4">
                {recipe.instructions}
              </p>
            </div>
...
const RecipeCard = ({ recipe, onDelete, onEdit, onViewDetails }) => (
  <div
    className="bg-slate-900/50 border border-slate-700 rounded-2xl overflow-hidden hover:border-purple-500/50 transition-all duration-200 group cursor-pointer hover:shadow-lg hover:shadow-purple-900/20"
    ...
    <div className="p-4 sm:p-5">
      ...
      {recipe.link && (
        <a
          ...
          rel="noreferrer noopener"
          onClick={e => e.stopPropagation()}
          className="mt-2 inline-flex items-center gap-1.5 text-sm text-purple-300 hover:text-purple-200 transition-colors"
        >
          <LinkIcon size={14} />
          Source
        </a>
      ...
      <p className="mt-3 text-xs text-slate-500 group-hover:text-slate-400 transition-colors">
