...
export const DEFAULT_DASHBOARD_SECTIONS = ['calendar', 'tasks', 'locations', 'expenses', 'recipes', 'watchlist'];
...
const normalizeExpense = (expense = {}) => {
  ...
  const category = VALID_EXPENSE_CATEGORIES.has(asString(item.category)) ? asString(item.category) : 'other';
...
export const initializeDatabase = async () => {
  try {
    ...
    await sql`
      ALTER TABLE dashboards
      ADD COLUMN IF NOT EXISTS enabled_sections JSONB NOT NULL DEFAULT '["calendar","tasks","locations","expenses","recipes","watchlist"]'::jsonb
    `;

    await sql`
      ALTER TABLE dashboards
      ALTER COLUMN enabled_sections SET DEFAULT '["calendar","tasks","locations","expenses","recipes","watchlist"]'::jsonb
    `;
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS dashboard_data (
        ...
        data JSONB NOT NULL DEFAULT '{"calendarEvents":[],"tasks":[],"locations":[],"expenses":[],"recipes":[],"watchlist":[],"profile":{"users":[]}}'::jsonb,
    ...
    await sql`
      ...
      ALTER COLUMN data SET DEFAULT '{"calendarEvents":[],"tasks":[],"locations":[],"expenses":[],"recipes":[],"watchlist":[],"profile":{"users":[]}}'::jsonb
    `;
    ...
    // Migrate trips -> expenses in existing dashboard_data JSONB rows
    await sql`
      UPDATE dashboard_data
      SET data = jsonb_set(
        data - 'trips',
        '{expenses}',
        COALESCE(data->'trips', '[]'::jsonb)
      )
      WHERE data ? 'trips' AND NOT (data ? 'expenses')
    `;

    // Migrate enabled_sections: replace 'trips' with 'expenses'
    await sql`
      UPDATE dashboards
      SET enabled_sections = (
        SELECT jsonb_agg(
          CASE WHEN elem::text = '"trips"' THEN '"expenses"'::jsonb ELSE elem END
        )
        FROM jsonb_array_elements(enabled_sections) AS elem
      )
      WHERE enabled_sections @> '["trips"]'::jsonb
    `;
