...
export const DEFAULT_SPACE_DATA = {
  calendarEvents: [],
  tasks: [],
  locations: [],
  trips: [],
...
export const DEFAULT_SPACE_SECTIONS = ['calendar', 'tasks', 'locations', 'trips', 'recipes', 'watchlist'];
...
export const normalizeSpaceSections = (sections) => {
  const allowed = new Set(DEFAULT_SPACE_SECTIONS);
  const normalized = asArray(sections).filter((section) => allowed.has(section));
  return normalized.length ? Array.from(new Set(normalized)) : [...DEFAULT_SPACE_SECTIONS];
};
...
export const normalizeSpaceData = (data = {}) => {
  ...
  return {
    ...
    profile: raw.profile && typeof raw.profile === 'object' ? normalizeProfile(raw.profile) : DEFAULT_SPACE_DATA.profile
  };
};
...
export const initializeDatabase = async () => {
  try {
    ...
    // Migrate legacy table names to new naming convention
    ...
    if (shelvesRelationKind === 'r' && !shelfIdRelationKind && !spacesRelationKind) {
      await sql`ALTER TABLE shelves RENAME TO spaces`;
    } else if (shelfIdRelationKind === 'r' && !spacesRelationKind) {
      await sql`ALTER TABLE shelf_id RENAME TO spaces`;
    }
    ...
    if (shelfMembersRelationKind === 'r' && !spaceMembersRelationKind) {
      await sql`ALTER TABLE shelf_members RENAME TO space_members`;
    }
    ...
    if (shelfJoinCodesRelationKind === 'r' && !spaceJoinCodesRelationKind) {
      await sql`ALTER TABLE shelf_join_codes RENAME TO space_join_codes`;
    }

    const spaceDataRelationKind = await getRelationKind('space_data');
    const shelfDataRelationKind = await getRelationKind('shelf_data');
    if (shelfDataRelationKind === 'r' && !spaceDataRelationKind) {
      await sql`ALTER TABLE shelf_data RENAME TO space_data`;
    }

    // Migrate legacy shelf_id column name to space_id across junction tables
    const hasShelfIdInMembers = await sql`
      SELECT 1 FROM information_schema.columns
      WHERE table_name = 'space_members' AND column_name = 'shelf_id'
      LIMIT 1
    `;
    if (hasShelfIdInMembers.rows.length > 0) {
      await sql`ALTER TABLE space_members RENAME COLUMN shelf_id TO space_id`;
    }

    const hasShelfIdInJoinCodes = await sql`
      SELECT 1 FROM information_schema.columns
      WHERE table_name = 'space_join_codes' AND column_name = 'shelf_id'
      LIMIT 1
    `;
    if (hasShelfIdInJoinCodes.rows.length > 0) {
      await sql`ALTER TABLE space_join_codes RENAME COLUMN shelf_id TO space_id`;
    }

    const hasShelfIdInData = await sql`
      SELECT 1 FROM information_schema.columns
      WHERE table_name = 'space_data' AND column_name = 'shelf_id'
      LIMIT 1
    `;
    if (hasShelfIdInData.rows.length > 0) {
      await sql`ALTER TABLE space_data RENAME COLUMN shelf_id TO space_id`;
    }

    // Space metadata table (formerly shelf_id)
    await sql`
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_space_join_codes_code ON space_join_codes(code)`;
