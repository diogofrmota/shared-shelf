...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (confirmedUser && await canSendNonEssentialEmail(confirmedUser.id)) {
      const preferencesUrl = emailPreferencesUrl(confirmedUser.id);
      try {
        const resend = await sendResendEmail({
          to: confirmedUser.email,
          template: welcomeEmail({
            displayName: confirmedUser.display_name,
            appUrl: APP_URL,
            preferencesUrl
          })
        });
        if (resend === null) {
          logMissingEmailConfig('Welcome Email', confirmedUser.email, APP_URL);
        }
      } catch (emailError) {
